...
test.describe('Complete E2E Workflow', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript((mockTracks) => {
      ...
      (globalThis as unknown).__TEST_MOCK_TRACKS__ = mockTracks;
    }, testMockTracks);
  });

  test('complete end-to-end workflow: auth → add 4 players with songs → lineup → game', async ({ page }) => {
    // 1. Authentication
    ...
    const battingOrder = [player1.name, player2.name, player3.name, player4.name];
    ...
    if (await gamePage.arePlaybackControlsVisible() && await gamePage.isPlayButtonVisible()) {
